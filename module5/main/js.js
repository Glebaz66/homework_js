...
// */
const getAllSkills = arr => arr.reduce((acc,user) => acc.concat(user.skills), [])
...
const getUserNamesSortedByFriendsCount = arr => {
   const arr1 = arr.reduce((acc, val) => ({...acc,[val.name]: val.friends.length}),{});
   const arr2 = Object.entries(arr1);
   return arr2;
};
